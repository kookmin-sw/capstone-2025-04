...
def check_localstack_running():
    ...
    try:
        result = subprocess.run(
            ["docker", "ps", "--filter", "name=problem-generator-localstack"],
            stdout=subprocess.PIPE,
            text=True,
            check=False,
        )
...
def setup_resources():
    ...
    if not check_localstack_running():
        ...
        if not start_localstack():
            print(
                "Error: Could not start LocalStack. Please check your Docker installation."
            )
            return False
    ...
    if not wait_for_localstack():
        print(
            "Error: LocalStack is not ready after waiting. Please check LocalStack status."
        )
        return False
...
def submit_job(algorithm_type, difficulty):
    ...
    result_object_key = f"results/{problemId}.json"
    ...
    # DynamoDB 상태 초기화
    add_job_status(problemId, algorithm_type, difficulty)

    return problemId, result_url
...
def process_queue_locally(api_key):
    ...
    if not all([problemId, algorithm_type, difficulty, result_object_key]):
        print(f"Invalid message format: {body}. Skipping.")
        # 잘못된 메시지는 큐에서 삭제할 수 있음 (선택 사항)
        # delete_message_from_sqs(receipt_handle)
        return

    print(f"Processing job: {problemId}")

    try:
        # DynamoDB 상태를 PROCESSING으로 업데이트
        update_job_status(problemId, "PROCESSING")
        print(f"[{problemId}] DynamoDB status updated to PROCESSING.")

        # 문제 생성 호출
        print(
            f"[{problemId}] Calling generate_problem with algorithm_type={algorithm_type}, difficulty={difficulty}"
        )
        start_time = time.time()
        problem_result = generate_problem(
            api_key, algorithm_type, difficulty, verbose=True
        )
        end_time = time.time()
        print(
            f"[{problemId}] generate_problem finished in {end_time - start_time:.2f} seconds."
        )

        # 결과 S3에 업로드
        result_url = upload_to_s3(problem_result, object_key=result_object_key)
        print(f"[{problemId}] Result uploaded to S3: {result_url}")

        # DynamoDB 상태를 COMPLETED로 업데이트
        update_job_status(problemId, "COMPLETED", result_url=result_url)
        print(f"[{problemId}] DynamoDB status updated to COMPLETED.")

        # 메시지 삭제
        delete_message_from_sqs(receipt_handle)
        print(f"[{problemId}] Message deleted from SQS.")

    except Exception as e:
        print(f"[{problemId}] Error processing job: {str(e)}")
        # DynamoDB 상태를 FAILED로 업데이트
        update_job_status(problemId, "FAILED", error_message=str(e))
        print(f"[{problemId}] DynamoDB status updated to FAILED.")
        # 실패한 메시지는 큐에 남겨둠 (또는 데드 레터 큐로)


def main():
    parser = argparse.ArgumentParser(
        description="Simulate Problem Generator AWS locally using LocalStack"
    )
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Initialize AWS resources in LocalStack and exit",
    )
    parser.add_argument(
        "-a", "--algorithm", default="구현", help="Algorithm type for the problem"
    )
    parser.add_argument(
        "-d", "--difficulty", default="쉬움", help="Difficulty level for the problem"
    )
    parser.add_argument(
        "-k", "--api_key", help="Google AI API Key (overrides environment variable)"
    )
    ...
    # 작업 제출
    problemId, result_url = submit_job(args.algorithm, args.difficulty)
    print(
        f"==> Job Submitted: ID={problemId}, Algorithm='{args.algorithm}', Difficulty='{args.difficulty}'"
    )

    # DynamoDB에서 초기 상태 확인
    print(f"Checking initial status for job {problemId}...")
    time.sleep(1)  # DynamoDB 반영 시간 약간 대기
    initial_status = get_job_status(problemId)
    print(f"==> Initial DynamoDB Status: {initial_status}")

    # 큐 처리 시뮬레이션
    print(f"\nSimulating queue processing for job {problemId}...")
    time.sleep(2)  # 처리 시작 전 약간 대기
    process_queue_locally(api_key)

    # DynamoDB에서 최종 상태 확인
    print(f"\nChecking final status for job {problemId}...")
    time.sleep(1)
    final_status = get_job_status(problemId)
