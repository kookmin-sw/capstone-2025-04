...
def initialize_dynamodb_table(dynamodb_resource):
    ...
    try:
        table = dynamodb_resource.create_table(
            ...
            KeySchema=[{"AttributeName": "problemId", "KeyType": "HASH"}],  # 파티션 키
            AttributeDefinitions=[
                {"AttributeName": "problemId", "AttributeType": "S"}  # 문자열 타입
            ],
        ...
        table.meta.client.get_waiter("table_exists").wait(
            TableName=DYNAMODB_TABLE_NAME, WaiterConfig={"Delay": 1, "MaxAttempts": 10}
        )
        ...
        if error_code == "ResourceInUseException":
            ...
            print(
                f"LocalStack DynamoDB limit reached, assuming table '{DYNAMODB_TABLE_NAME}' exists."
            )
            return dynamodb_resource.Table(DYNAMODB_TABLE_NAME)
        else:
